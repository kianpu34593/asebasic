...
def create_element_dir(
    ...
    if "ads" in options:
        for struc in surf_struc:
            slab_db_path = (
                element + "/" + "surf" + "/" + struc + "/" + "layer_converge.db"
            )
            ...
            if os.path.isdir(element + "/" + "ads" + "/" + struc):
                print(
                    ...
                    + "ads"
                    + "/"
                    + "{} directory already exists!".format(struc)
                )
                pause()
            else:
...
def create_ads_and_dir(
    element, surf_struc, ads_atom=["Li"], ads_site=["ontop", "hollow", "bridge"]
):
    current_dir = os.getcwd()
    ...
    for struc in surf_struc:
        ...
        adsorption.generate_rxn_structures(
            surf, ads=ads_atom, site_type=ads_site, write_to_disk=True
        )
        os.chdir(current_dir)
...
def create_ads_sub_dir(element, struc, current_dir, site, ads_atom, slab_db):
    ...
    for i in range(db_size):
        ...
        adsorption.generate_rxn_structures(
            slab_db.get_atoms(i + 1), ads=ads_atom, site_type=site, write_to_disk=True
        )
...
def cif_grabber(API_key, pretty_formula):
    ...
    doc_ls = mpr.query(
        {"pretty_formula": pretty_formula}, ["material_id", "formation_energy_per_atom"]
    )
    form_dict = {}
    for doc in doc_ls:
    ...
    struc = mpr.get_structure_by_material_id(
        lowest_matID, final=True, conventional_unit_cell=True
    )
